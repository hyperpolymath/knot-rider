...
// Replace rsr-template-repo, knot-rider, Zig 0.16.0 with actuals
...
| `$PREFIX/share/knot-rider/`
| Data files, assets

| `$PREFIX/share/doc/knot-rider/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/knot-rider/config.toml`

Fallback: `$HOME/.config/knot-rider/config.toml`
...
Or via OPSM: `opsm update knot-rider`
...
[source,bash]
----
just install --prefix=/opt/knot-rider-instance1 --config=/etc/knot-rider/instance1.toml
just install --prefix=/opt/knot-rider-instance2 --config=/etc/knot-rider/instance2.toml
----
