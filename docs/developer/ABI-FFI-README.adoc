...
= ABI/FFI Standards
# knot-rider ABI/FFI Documentation
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libknot_rider.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── knot_rider.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── knot_rider.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/knot_rider.h
```
...
```c
#include "knot_rider.h"

int main() {
    void* handle = knot_rider_init();
    if (!handle) return 1;

    int result = knot_rider_process(handle, 42);
    if (result != 0) {
        const char* err = knot_rider_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    knot_rider_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lknot_rider -L./zig-out/lib
```
...
```rust
#[link(name = "knot_rider")]
extern "C" {
    fn knot_rider_init() -> *mut std::ffi::c_void;
    fn knot_rider_free(handle: *mut std::ffi::c_void);
    fn knot_rider_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = knot_rider_init();
        assert!(!handle.is_null());

        let result = knot_rider_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libknot_rider = "libknot_rider"

function init()
    handle = ccall((:knot_rider_init, libknot_rider), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:knot_rider_process, libknot_rider), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:knot_rider_free, libknot_rider), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/interface/Abi/Types.idr -o generated/abi/knot_rider.h
   ```
